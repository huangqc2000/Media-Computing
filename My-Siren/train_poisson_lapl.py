...
from utils import laplace, save_result
...
def img_laplace_mse(model_output, gt):
    channels = gt['img'].shape[-1]
    if channels == 1:
        laplacian = laplace(model_output['model_out'], model_output['model_in'])
    else:
        laplacian = []
        for c in range(channels):
            lapl_channel = laplace(model_output['model_out'][..., c], model_output['model_in'])
            laplacian += [lapl_channel]
        laplacian = torch.cat(laplacian, dim=-1)
    return torch.mean((laplacian - gt['laplace'])**2)
...
def main():
    ...
    loss_fn = img_laplace_mse
    print("Define Optimizer and Loss Function.")

    img_name = args.img_path.split('/')[-1][:-4]
    model_dir = os.path.join(os.path.join(args.result_dir, "poisson_laplace"), img_name)
